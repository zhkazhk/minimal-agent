...
| 自动化测试 | **253 passed / 0 failed**（10 个测试模块，含 11 个端到端验收用例） |
...
> 📄 **如果你是按交付要求验收本项目，建议先看 [`docs/SUBMISSION.md`](docs/SUBMISSION.md)** ——
> 它把四份提交材料（真实 LLM API 实证 / 代码链接 / README / AI Prompt 与问题记录）组织在一起，
> 并附上可直接复核的运行命令。
...
**本项目的 Memory = Session 内的完整消息列表；不做事前检索，每次 LLM 调用前全量注入。**
召回时机 = 每次 `ContextManager.build()`；放置方式 = `[system prompt] → [历史消息按时间升序] → [本轮用户消息]`。
...
| 触发点 | 时机 | 说明 |
| --- | --- | --- |
| ① 新一轮用户提问 | `run()` 进入 `while True` 后第一次 `build()` | 用户消息先 append 进 session，再连同全部历史一起组装 |
| ② 工具结果回灌后继续循环 | 同一轮内的第 2、3… 次 `build()` | **每调用一次 LLM 就重新召回一次**，模型因此能看到本轮已产生的工具结果 |
| ③ 追问（纯对话 / 带工具） | 与①完全同路径 | 追问只是「往同一 session 再 append 一条 user 消息」，无需额外逻辑 |

> 关键点：**召回发生在「每次 LLM 调用」之前，而不是「每轮对话」之前。**
> 一次提问若触发 3 次工具调用，`build()` 会执行 4 次（首次 + 3 次回灌后），每次注入的历史都比上次更长
> —— 这正是 Step6「回到 Step2」在 Memory 层面的体现。

**实证**（`docs/evidence/memory_trace.md`：真实模型三轮追问，逐轮记录实际发出的请求）

| 轮次 | 用户说 | session 已有 | 实际发出的首条请求 | token 估算 |
| --- | --- | --- | --- | --- |
| 1 | `上海今天天气` | 0 条 | **2 条**（system + 本轮问题） | 1979 |
| 2 | `那明天呢` | 4 条 | **6 条**（system + 4 历史 + 本轮问题） | 2474 |
| 3 | `那北京呢` | 8 条 | **10 条**（system + 8 历史 + 本轮问题） | 2703 |

第 2 轮「那明天呢」既没提城市也没提"天气"，模型仍准确答出「上海明天阴天」——
唯一来源就是被召回的第 1 轮历史。这是 Memory 生效的直接证明。

### 8.4 放置方式（Where）

```
┌──────────────────────────────────────────────────────────────┐
│ 1. system prompt        ← 每次实时拼接，不落 session           │
│    · JSON 协议约束                                             │
│    · 工具列表 + 参数 Schema（ToolRegistry.snapshot() 注入）     │
│    · 会话信息 / 当前时间 / 轮次上限                             │
│    · 运行时临时规则（轮次告警、解析失败提示、重复调用提示）       │
├──────────────────────────────────────────────────────────────┤
│ 2. 【上下文已裁剪…】     ← 仅发生裁剪时插入（system 角色）       │
├──────────────────────────────────────────────────────────────┤
│ 3. 历史消息（时间升序）                                         │
│    · user        → 原样                                        │
│    · assistant   → 原样（含思考文本与最终答案）                  │
│    · tool_call   → 渲染成 assistant 的 JSON 协议文本            │
│    · tool_result → 渲染成 user 的「【工具结果 <name>】\n…」      │
│    · error       → 渲染成 user 的「【错误 <name>】\n…」          │
├──────────────────────────────────────────────────────────────┤
│ 4. 本轮用户消息（已 append 到 session 尾部）                    │
└──────────────────────────────────────────────────────────────┘
```

**为什么这样放**

- `system` 放最前且**不落 session** —— 否则历史里会堆叠几十份重复指令，既浪费 token 也让模型行为漂移；
- 工具 Schema 只在 system 里出现一次 —— 不重复占位；
- 工具调用与结果**渲染成纯文本协议**而非原生 function calling —— 任何最简 chat API 都能跑，
  且模型能从历史里"看到"自己当初调了什么、拿到了什么（这是「带工具追问」能成立的前提）；
- 裁剪提示紧跟 system —— 让模型立刻知道"前面有缺失"，而不是读到一半才发现指代不明。

**副产品：前缀稳定 → 命中 Prompt Cache。**
system prompt 按「固定模板 + 排序后的工具列表」渲染（`ToolRegistry.snapshot()` 按名字排序），
前缀在多次请求间完全一致。实测 DeepSeek 返回
`prompt_cache_hit_tokens: 1408`（首轮）/ `1536`（次轮），约 **87% 的输入 token 走了缓存** —— 长对话更省也更稳。
证据：`docs/evidence/real_llm_run.md`。

### 8.5 为什么不做事前检索（No Retrieval）

本项目的目标是「最小可用 Agent Runtime」。记忆召回会引入 embedding、向量检索、写入策略、遗忘策略
等一整套复杂度，与「把 Agent 主循环写清楚」的目标冲突。
当前设计里 session 消息既是最简单的记忆，也是唯一的事实来源 —— 简单、可调试、行为可预测。

### 8.6 升级路径（不改主循环，只替换组件）

1. `SessionStore` → Redis / Postgres，实现跨进程与多实例共享；
2. `ContextManager.build()` 里把「丢弃最早消息」换成「调用 LLM 生成摘要」写入
   `session.meta["running_summary"]`，放在裁剪提示的位置（system 之后、历史之前）；
3. 需要长期记忆时，在 `ContextManager` 中新增「检索并注入」步骤
   （召回结果作为 `system` 或 `user` 消息插到历史之前），主循环与协议层无需改动。
